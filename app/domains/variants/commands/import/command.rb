  ...
  module Commands
    module Import
      class Command < ::RDiplomacy::Command
        ...
        #
        def perform(request)
          variant_yml = yield load_variant_yml(request.abbr)
        ...
        # @return [Failure<Error>]
        #
        def load_variant_yml(variant_abbr)
          variant_file = Rails.root.join('app', 'configuration', 'variants', "#{variant_abbr}.yml")
          logger.info "Loading variant configuration at #{variant_file}"
          Success(YAML.safe_load_file(variant_file))
        rescue StandardError => e
          logger.error "Failed to import variant #{variant_abbr}: #{e.message}"
